...
#VBA version. This requires a command line argument of the path to the PF_PM_Data_Collection_Workbook
#
...
#Next, open Avatar to the Referral Source Maintenance form and don't click anything. 
#After starting the script from the workbook this script will read the macroReferral_Source sheet in the workbook. 
# If there are any problems during this operation such as the workbook not being available to open, or the sheet
# not being found, the script will display a meesage box informing the user of the error and will safely exit.
...
# box the script will being the automated process of entering in all of the referral sources listed in the Referral_Source sheet in 
# the workbook. If the script needs to be stopped for any reason the user can close the command prompt window that the .bat file opened.
# This will kill the automation process.
...
#Create the PowerShell modules directory and place the WASP automation .dll into the new directory so this script can use it. 
# If the path already exists, do nothing.
If (!(Test-Path $toLoc))
{
    New-Item $toLoc -type directory -force
...
#Get the file path to the workbook from the command line argument.
$filePath = $args[0]
